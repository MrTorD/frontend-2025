import type { Presentation, Slide, TextField, Color, Picture, ImageField, Object } from "./Types";
...
export function changeSlidePosition(p: Presentation, sId1: string, sId2: string) : Presentation {
...
export function addText(p: Presentation, sId: string, x: number, y: number, w: number, h: number, text: string, font: string, size: number) : Presentation {
...
export function deleteObj(p: Presentation, sId: string, objId: string) : Presentation {
    const slides = p.slides;
...
export function addImage(p: Presentation, sId: string, x: number, y: number, w: number, h: number, src: string) : Presentation {
    const obj: Object = {x, y, w, h, id: crypto.randomUUID()};
...
export function changeObjPositon(p: Presentation, sId: string, oId: string, x: number, y:number, w: number, h:number) : Presentation {
    const slides = p.slides;
...
export function editText(p: Presentation, sId: string, tId: string, text: string) : Presentation {
    const slides = p.slides;
    const slide = slides.find(s => s.id = sId);
...
export function changeTextSize(p: Presentation, sId: string, tId: string, size: number) : Presentation {
    const slides = p.slides;
    const slide = slides.find(s => s.id = sId);
    let t = slide?.slideObjects.find(t => t.id == tId);
...
export function changeFont(p: Presentation, sId: string, tId: string, font: string) : Presentation {
    const slides = p.slides;
    const slide = slides.find(s => s.id = sId);
    let t = slide?.slideObjects.find(t => t.id == tId);
...
export function changeBackground(p: Presentation, sId: string, src: string, type: "color" | "picture") : Presentation {
    const slides = p.slides;
    const slide = slides.find(s => s.id = sId);
    if (slide) {
